...
for idxi in idx_full:
    gene_pairs_ = pd.read_csv(
        Path.joinpath(WORKDIR, "results", "svgbit", f"{idxi}-gene_pairs.csv"),
        index_col=0,
        header=0,
    )
    gene_pairs_ = gene_pairs_[gene_pairs_["gene_1"] == "VIM"]
    gene_pairs_ = gene_pairs_[gene_pairs_["colocalization_degree"] == "high"]

# %%
genes = [
    "PTPRZ1", "IGFBP2", "TP53", "KIF17", "TMEM176B", "DDOST", "SOCS2", "NES",
    "TNFRSF21", "EDNRB", "NMB", "METRN", "FADS2", "LANCL2", "S100A16", "FCGBP",
    "CRISPLD1", "IGFBP5", "TMEM176A", "MUL1", "PMP2"
]
save_dir = Path.joinpath(WORKDIR, "results", "svgbit", idx)
if not save_dir.exists():
    save_dir.mkdir()
for gene in genes:
    sb.plot.hotspot_colocalization_map(
        d,
        ("VIM", gene),
        Path.joinpath(WORKDIR, f"{idx}-VIM-{gene}.svg"),
        he_path,
        s=8,
    )
